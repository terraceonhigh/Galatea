...
// doServe stands the lifted NFSv4 server up on a loopback TCP port and serves
// the demo tree until interrupted. This is the R3 → R4 bridge: a real socket a
// macOS NFS client can connect to. Confirming the Finder-visible mount itself
// (R4) needs a non-headless Mac.
func doServe(addr string) error {
	root := demoTree()
	program := nfssrv.NewReadOnlyProgram(root, virtual.NewMemoryHandleResolver(root))
	...
	fmt.Printf("galatea: serving NFSv4 (read-only demo tree) on %s\n", ln.Addr())
