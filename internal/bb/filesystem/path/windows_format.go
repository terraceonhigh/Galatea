...
func parseUNCPath(uncPath string, scopeWalker ScopeWalker) (ComponentWalker, RelativeParser, error) {
	...
	if serverLen == -1 {
		return nil, nil, errors.New("Invalid UNC path: expected a non-empty server and share name")
	}
	if serverLen < 1 {
		return nil, nil, errors.New("Invalid UNC path: expected a non-empty server name")
	}
	...
	if shareLen < 1 {
		return nil, nil, errors.New("Invalid UNC path: expected a non-empty share name")
	}
...
func (rp windowsRelativeParser) ParseFirstComponent(componentWalker ComponentWalker, mustBeDirectory bool) (next GotDirectoryOrSymlink, remainder RelativeParser, err error) {
	...
	if err := validateWindowsComponent(name); err != nil {
		return nil, nil, fmt.Errorf("Invalid pathname component %#v: %w", name, err)
	}
