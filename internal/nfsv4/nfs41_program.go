...
func (p *nfs41Program) opSequence(ctx context.Context, args *nfsv4.Sequence4args, argArray []nfsv4.NfsArgop4) compoundResult {
	...
	case slot.lastSequenceID + 1:
		...
		if slot.currentSequenceWaiters != nil {
			...
			// Galatea fix (R2d): upstream bb-rex wrote `append(slot.current`
			// `SequenceWaiters)` with no value — a no-op, so `ch` was never
			// registered and this goroutine would block forever on `<-ch`
			// under a concurrent same-slot SEQUENCE (the completion side at
			// "Broadcast result to parallel calls" drains this slice). `go vet`
			// flagged the empty append. See docs/MISTAKES.md (M-005).
			slot.currentSequenceWaiters = append(slot.currentSequenceWaiters, ch)
			p.leave()
			return <-ch
		}
